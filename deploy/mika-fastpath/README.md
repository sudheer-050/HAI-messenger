...
| `audit.js` | Formats and posts the mandatory audit comment (success, rollback, or rejection) via the `multica` CLI — reuses the CLI install/auth the Mika bridge already requires, no new credential surface. Also holds `writeAuditOutbox`, the durable local fallback used when posting to Multica itself fails (see MYAG-205 below). |
| `orchestrator.js` | Ties the above together: diff → carve-out check → deploy.sh → health-check → rollback.sh if unhealthy → a fresh post-rollback health check → exactly one audit comment, always. Rollback is only ever reported as `rolled_back` once that post-rollback check actually passes; `rollback_unverified` (rollback ran, restored service still fails health) and `rollback_failed` (rollback.sh itself errored) are distinct outcomes with their own exit codes (4 and 5) and audit wording — see MYAG-204. Filing the carve-out reroute issue and posting the audit comment are independent: a reroute-filing failure is surfaced *inside* the audit comment rather than aborting it, and if posting the audit comment itself fails, the record is written to `deploy/mika-fastpath/.audit-outbox/` (gitignored) instead of being lost — see MYAG-205. |
