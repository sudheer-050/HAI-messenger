...
/* ---- Job sourcing: Adzuna (primary, needs a free API key) + RemoteOK (free,
   ...
   rather than out to the company. There's no way around that server-side, so
   the `source` field is used client-side to label RemoteOK results "via
   RemoteOK" instead of presenting the link as the company's own.
   Results are cached in job_postings_cache per normalized role query so N
   users tracking the same role share one Adzuna call instead of one each --
   Adzuna's free tier is only ~1,000 calls/month. ---- */
    ...
    try {
        ...
        return (Array.isArray(data) ? data : [])
            ...
            .map(j => ({
                ...
                remote: true,
                description: j.description || '',
                // Always a remoteok.com URL -- RemoteOK's API has no field for
                // the company's own apply page. The UI must label this
                // "via RemoteOK", never "Apply on {company}'s site".
